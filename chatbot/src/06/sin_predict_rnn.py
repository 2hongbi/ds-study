# ex.6-5 (p.181) sin 곡선 예측 RNN 모델 사용
import numpy as np
...
# 모델 학습
np.random.seed(0)
from tensorflow.keras.callbacks import EarlyStopping    # 조기 종료 콜백 객체 사용
early_stopping = EarlyStopping(monitor='loss', patience=5, mode='auto')
history = model.fit(train_x, train_y, epochs=1000, callbacks=[early_stopping])

# 학습 과정에 발생한 loss 그래프 생성
...
# cos()를 이용해 테스트용 데이터셋을 만드는 이유는 학습된 sin 파형과 주기적 차이를 주기 위해서
