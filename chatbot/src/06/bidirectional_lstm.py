# ex.6-7 (p.197) 양방향 LSTM
# 랜덤으로 시퀀스를 생성해 임의의 분류 기준에 맞는 클래스를 예측하는 양방향 LSTM
import numpy as np
...
def get_sequence(n_timesteps):
    ...
    y = np.array([0 if x < limit else 1 for x in np.cumsum(X)])     # cumsum : 배열에서 주어진 축에 따라 누적되는 원소들의 누적합
...
# 양방향 LSTM 모델 정의
model = Sequential()
# return_sequences=True : 정방향, 역방향 LSTM 계층에 모든 출력밧ㅇ
